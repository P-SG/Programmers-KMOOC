...
class KmoocRepository {
    ...
    private fun parseLectureList(jsonObject: JSONObject) = jsonObject.getJSONObject("pagination")
            .run {
                LectureList(
                    getInt("count"),
                    getInt("num_pages"),
    ...
    private fun parseLecture(jsonObject: JSONObject) = jsonObject.run {
            Lecture(
                getString("id"),
                getString("number"),
                getString("name"),
                ...
                if (has("teachers")) getString("teachers") else "",
                if (has("overview")) getString("overview") else "",
            )
        }
